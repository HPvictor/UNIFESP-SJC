...
const char * InstrNames[] =   { "nop", "halt", "add", "sub",   //1
                            "addi", "subi", "mult", "xor", //2
                            "and", "not", "or", "slt",    //3
                            "shftl", "shftr", "li", "sw", //4
                            "beq", "bne", "j", "jr",     //5
                            "in", "out", "sgt", "move",  //6
...
void insertVar (char * scope, char * id, int size, VariableKind kind) {
    FunList f = funlisthead;
...
VariableKind checkType (QuadList l) {
    QuadList aux = l;
    Quad q = aux->quad;
    aux = aux->next;
...
int getVarPosition (char * id, char * scope) {
    FunList f = funlisthead;
    while (f != NULL && strcmp(f->id, scope) != 0) f = f->next;
    if (f == NULL) {
        return -1;
...
VariableKind getVariableKind (char * id, char * scope) {
    FunList f = funlisthead;
    while (f != NULL && strcmp(f->id, scope) != 0) f = f->next;
    if (f == NULL) {
        return simple;
        ...
        switch (q.op) {
          ...
          case opIMMED:
            ...
            case opLOAD:
                ...
                aux = getVarPosition(a2.contents.var.name, a2.contents.var.scope);
                if (aux == -1)
                {
                    v = getVariableKind(a2.contents.var.name, "Global");
                    aux = getVarPosition(a2.contents.var.name, "Global");
                    if (v == vector)
                    {
                ...
                {
                    v = getVariableKind(a2.contents.var.name, a2.contents.var.scope);
                    if (v == vector)
                    {
            ...
            case opSTORE:
                aux = getVarPosition(a1.contents.var.name, a1.contents.var.scope);
                if (aux == -1)
                {
                    aux = getVarPosition(a1.contents.var.name, "Global");
                    if (a2.kind == String)
            ...
            case opVEC:
                v = getVariableKind(a2.contents.var.name, a2.contents.var.scope);
                if (v == simple) v = getVariableKind(a2.contents.var.name, "Global");
                aux = getVarPosition(a2.contents.var.name, a2.contents.var.scope);
                if (v == vector) {
                    if (aux == -1) {
                        aux = getVarPosition(a2.contents.var.name, "Global");
            ...
            case opARG:
                ...
                instructionFormat2(str, getArgReg(), $sp, getVarPosition(a1.contents.var.name, a3.contents.var.name), NULL);
                curarg ++;
                break;
